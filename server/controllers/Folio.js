...
const folioPage = (req, res) => {
  return res.render('folio', { csrfToken: req.csrfToken() });
};

const getPortal = (req, res) => {
  return Portal.PortalModel.findById(req.params.id, (err, doc) => {
    if (err) {
...
const getFolios = (req, res) => {
  return Folio.FolioModel.findByPortalId(req.params.portalId, (err, docs) => {
    if (err) {
      return res.status(400).json({ error: 'An error occurred' });
    }
